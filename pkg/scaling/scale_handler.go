...
type ScaleHandler interface {
	...
	GetScalersCache(scalableObject interface{}) (*ScalersCache, error)
}
...
type scaleHandler struct {
	...
	scalerCaches      map[string]*ScalersCache
	lock              *sync.RWMutex
}
...
func (h *scaleHandler) startScaleLoop(ctx context.Context, withTriggers *kedav1alpha1.WithTriggers, scalableObject interface{}, scalingMutex sync.Locker) {
	...
	for {
		...
		case <-ctx.Done():
			...
			h.ClearScalersCache(scalableObject)
			tmr.Stop()
			return
		}
	}
...
func (h *scaleHandler) GetScalersCache(scalableObject interface{}) (*ScalersCache, error) {
	withTriggers, err := asDuckWithTriggers(scalableObject)
	if err != nil {
		return nil, err
	}

	key := fmt.Sprintf("%s.%s", withTriggers.Name, withTriggers.Namespace)
	h.lock.RLock()
	...
	scalers, builders, err := h.buildScalers(withTriggers, podTemplateSpec, containerName)
	if err != nil {
		h.logger.Error(err, "error building some scalers")
	}

	h.lock.Lock()
	h.scalerCaches[key] = &ScalersCache{
		scalers: scalers,
		builders: builders,
		logger: h.logger,
		object: withTriggers,
...
func (h *scaleHandler) ClearScalersCache(scalableObject interface{}) {
	withTriggers, err := asDuckWithTriggers(scalableObject)
	if err != nil {
		return
	}
...
func (h *scaleHandler) checkScalers(ctx context.Context, scalableObject interface{}, scalingMutex sync.Locker) {
	...
	case *kedav1alpha1.ScaledObject:
		...
		isActive, isError, _ := cache.IsScaledObjectActive(ctx)
...
func (h *scaleHandler) buildScalers(withTriggers *kedav1alpha1.WithTriggers, podTemplateSpec *corev1.PodTemplateSpec, containerName string) ([]scalers.Scaler, []func() (scalers.Scaler, error), error) {
	...
	var scalersCtor []func() (scalers.Scaler, error)
	var err error
	resolvedEnv := make(map[string]string)

	for i, t := range withTriggers.Spec.Triggers {
		triggerName, trigger := i, t
		ctor := func() (scalers.Scaler, error) {
		...
		scalersCtor = append(scalersCtor, ctor)
	}

	return scalersRes, scalersCtor, nil
}
